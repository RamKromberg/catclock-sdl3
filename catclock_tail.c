/* ============================================================================
   FILE: catclock_tail.c (Decoupled Angle Tuning Shader)
   ============================================================================ */
...
void CatClock_ShaderTail(SDL_Renderer *renderer, int cell_w, int cell_h, float scale, int frame_idx, void *userdata) {
    ...
    float final_angle_deg = MAX_LEFT_ANGLE_DEG + (normalized_wave * (MAX_RIGHT_ANGLE_DEG - MAX_LEFT_ANGLE_DEG));
    ...
    /* ADJUSTMENT: Applied -1.0px leftward bias to center the origin and prevent right-swing scissoring! */
    ...
    vertices[v_ptr++].position = (SDL_FPoint){ pivot_x + (s0_x * cos_a - s0_y * sin_a), pivot_y + (s0_x * sin_a + s0_y * cos_a) };
    vertices[v_ptr++].position = (SDL_FPoint){ pivot_x + (s1_x * cos_a - s1_y * sin_a), pivot_y + (s1_x * sin_a + s1_y * cos_a) };
    vertices[v_ptr++].position = (SDL_FPoint){ pivot_x + (s2_x * cos_a - s2_y * sin_a), pivot_y + (s2_x * sin_a + s2_y * cos_a) };
    vertices[v_ptr++].position = (SDL_FPoint){ pivot_x + (s3_x * cos_a - s3_y * sin_a), pivot_y + (s3_x * sin_a + s3_y * cos_a) };
    ...
    for (int i = 0; i < RING_SEGMENTS; i++) {
        ...
        vertices[v_ptr++].position = (SDL_FPoint){ pivot_x + (ox * cos_a - oy * sin_a), pivot_y + (ox * sin_a + oy * cos_a) };
        vertices[v_ptr++].position = (SDL_FPoint){ pivot_x + (ix * cos_a - iy * sin_a), pivot_y + (ix * sin_a + iy * cos_a) };
    }
    ...
    vertices[v_ptr++].position = (SDL_FPoint){ pivot_x + (cap_center_x * cos_a - cap_center_y * sin_a), pivot_y + (cap_center_x * sin_a + cap_center_y * cos_a) };
    ...
    for (int i = 0; i < CAP_SEGMENTS; i++) {
        ...
        vertices[v_ptr++].position = (SDL_FPoint){ pivot_x + (cx * cos_a - cy * sin_a), pivot_y + (cx * sin_a + cy * cos_a) };
    }
    ...
    if (indices) {
        ...
        for (int i = 0; i < RING_SEGMENTS - 1; i++) {
            ...
            indices[idx++] = curr_o; indices[idx++] = curr_i; indices[idx++] = next_o;
            indices[idx++] = curr_i; indices[idx++] = next_i; indices[idx++] = next_o;
        }
