...
SDL_Texture* CatClock_LoadDynamicXbmToRGBA4444(SDL_Renderer* renderer, const char* path,
											   SDL_Color color, bool invert_mask);

static void BuildRuntimeNodeTextureRGBA4444(SDL_Renderer* renderer, XbmAssetNode* node,
											const char* path, const char* name, SDL_Color color,
											bool invert_mask) {
	if (!node || !path || !name)
		return;
...
CatClock_XbmLibrary* CatClock_InitXbmLibrary(SDL_Renderer* renderer) {
	...
	BuildRuntimeNodeTextureRGBA4444(renderer, &lib->catback, "./assets/catback.xbm", "catback",
									texture_mask_base, false);
	BuildRuntimeNodeTextureRGBA4444(renderer, &lib->catwhite, "./assets/catwhite.xbm", "catwhite",
									texture_mask_base, false);
	BuildRuntimeNodeTextureRGBA4444(renderer, &lib->cattie, "./assets/cattie.xbm", "cattie",
									neck_tie, false);
	BuildRuntimeNodeTextureRGBA4444(renderer, &lib->eyes, "./assets/eyes.xbm", "eyes", eyes_mask,
									true);
...
void CatClock_RenderXbmLayerOffset(CatClock_XbmLibrary* lib, SDL_Renderer* renderer,
	...
	if (strcmp(layer_id, "catback") == 0)
		target_node = &lib->catback;
	else if (strcmp(layer_id, "catwhite") == 0)
		target_node = &lib->catwhite;
	else if (strcmp(layer_id, "cattie") == 0)
		target_node = &lib->cattie;
	else if (strcmp(layer_id, "eyes") == 0)
		target_node = &lib->eyes;
	...
	if (target_node && target_node->texture) {
		...
		if (strcmp(layer_id, "eyes") == 0) {
			...
			dst.w = (float) target_node->w;
			dst.h = (float) target_node->h;
		} else {
			dst.x = offset_x;
			dst.y = offset_y;
...
void CatClock_DestroyXbmLibrary(CatClock_XbmLibrary* lib) {
	if (!lib)
		return;
	if (lib->catback.texture)
		SDL_DestroyTexture(lib->catback.texture);
	if (lib->catwhite.texture)
		SDL_DestroyTexture(lib->catwhite.texture);
	if (lib->cattie.texture)
		SDL_DestroyTexture(lib->cattie.texture);
	if (lib->eyes.texture)
		SDL_DestroyTexture(lib->eyes.texture);
	free(lib);
}
...
void CatClock_SetXbmLayerScaleMode(CatClock_XbmLibrary* lib, const char* layer_id,
								   SDL_ScaleMode mode) {
	if (!lib || !layer_id)
		return;
...
void CatClock_SetXbmLayerBlendMode(CatClock_XbmLibrary* lib, const char* layer_id,
								   SDL_BlendMode mode) {
	if (!lib || !layer_id)
		return;
...
void CatClock_RenderHaloOutline(CatClock_XbmLibrary* lib, SDL_Renderer* renderer, SDL_Color color) {
	if (!lib || !renderer || !lib->catback.texture)
		return;
